                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 z401K ought to be enough for anybody." - Bill Gates, 2017
    ui->z401KpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelz401KSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->z401KpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintz401K_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z401K is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " z401K...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" z401K in ") +
...
void PrivacyDialog::on_pushButtonSpendz401K_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z401K is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn z401K
        sendz401K();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send z401K
    sendz401K();
}

void PrivacyDialog::on_pushButtonZ401kControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    Z401kControlDialog* z401kControl = new Z401kControlDialog(this);
    z401kControl->setModel(walletModel);
    z401kControl->exec();
}

void PrivacyDialog::setZ401kControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelz401KSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendz401K()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid 401KCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->z401KpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as z401K is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " 401K </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " z401K</b>";
    ...
    // use mints from z401K selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!Z401kControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = Z401kControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 z401K require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend z401K"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == Z401K_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 z401K require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend z401K"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If z401K was spent successfully update the addressbook with the label
    ...
    // Clear z401k selector in case it was used
    Z401kControlDialog::setSelectedMints.clear();
    ui->labelz401KSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("z401K Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " 401k, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("z401K Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->z401kTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " z401K </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" z401K "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" z401K "));
    ui->labelz401KAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelz401KSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintz401K->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint z401K
        ui->pushButtonMintz401K->setEnabled(false);
        ui->pushButtonMintz401K->setToolTip(tr("z401K is currently disabled due to maintenance."));

        // Spend z401K
        ui->pushButtonSpendz401K->setEnabled(false);
        ui->pushButtonSpendz401K->setToolTip(tr("z401K is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint z401K
        ui->pushButtonMintz401K->setEnabled(true);
        ui->pushButtonMintz401K->setToolTip(tr("PrivacyDialog", "Enter an amount of 401K to convert to z401K", 0));

        // Spend z401K
        ui->pushButtonSpendz401K->setEnabled(true);
        ui->pushButtonSpendz401K->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}