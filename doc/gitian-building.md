...
*Setup instructions for a gitian build of 401KCoin using a Debian VM or physical system.*
...
- [Building 401KCoin](#building-401kcoin)
...
Clone the git repositories for 401kcoin and gitian and then checkout the 401kcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 401kcoin repository
...
To build 401KCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 401kcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/401kcoin/.git/
...
For example:
```bash
URL=https://github.com/crowning-/401kcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/401kcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/401kcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/401kcoin-build.assert
```
...
[401kcoin/gitian.sigs](https://github.com/401kcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
