...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.z401kTracker = unique_ptr<Cz401KTracker>(new Cz401KTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == Z401K_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == Z401K_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
