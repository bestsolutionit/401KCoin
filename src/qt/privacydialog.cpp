...
void PrivacyDialog::sendzPIV()
{
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " 401KCoin, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>z401K </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " z401K </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("z401K is currently disabled due to maintenance."));

        // Spend zPIV
        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("z401K is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zPIV
        ui->pushButtonMintzPIV->setEnabled(true);
        ui->pushButtonMintzPIV->setToolTip(tr("PrivacyDialog", "Enter an amount of 401K to convert to z401K", 0));
