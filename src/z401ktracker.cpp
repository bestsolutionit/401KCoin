...
Cz401KTracker::Cz401KTracker(std::string strWalletFile)
{
...
Cz401KTracker::~Cz401KTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void Cz401KTracker::Init()
{
...
bool Cz401KTracker::Archive(CMintMeta& meta)
{
...
bool Cz401KTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta Cz401KTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta Cz401KTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool Cz401KTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> Cz401KTracker::GetSerialHashes()
{
...
CAmount Cz401KTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount Cz401KTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> Cz401KTracker::GetMints(bool fConfirmedOnly) const
{
...
bool Cz401KTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool Cz401KTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool Cz401KTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool Cz401KTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool Cz401KTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool Cz401KTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool Cz401KTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void Cz401KTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz401KTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz401KTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void Cz401KTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void Cz401KTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool Cz401KTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> Cz401KTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dz401k from DB\n", __func__, listDeterministicDB.size());
    }
