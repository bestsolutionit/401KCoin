...
Cz401KWallet::Cz401KWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing z401k seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZ401KSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZ401KSeed_deprecated()) {
                    LogPrintf("%s: Updated z401K seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old z401k seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of z401k wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool Cz401KWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZ401KCount(nCountLastUsed);
    else if (!walletdb.ReadZ401KCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void Cz401KWallet::Lock()
{
    seedMaster = 0;
}

void Cz401KWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void Cz401KWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZ401K(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool Cz401KWallet::LoadMintPoolFromDB()
{
...
void Cz401KWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void Cz401KWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void Cz401KWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->z401kTracker->HasPubcoinHash(pMint.first)) {
...
bool Cz401KWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZ401K(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to z401kTracker which also adds to database
    pwalletMain->z401kTracker->Add(dMint, true);
...
void Cz401KWallet::SeedToZ401K(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 Cz401KWallet::GetZerocoinSeed(uint32_t n)
{
...
void Cz401KWallet::GenerateDeterministicZ401K(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void Cz401KWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool Cz401KWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
