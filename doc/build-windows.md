...
Below are some notes on how to build 401KCoin Core for Windows.

The options known to work for building 401KCoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the 401KCoin Core Windows release binaries.
...
Note that for WSL the 401KCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/401kcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/401kcoin/401kcoin.git
    sudo chmod -R a+rw 401kcoin
...
way. This will install to `c:\workspace\401kcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/401kcoin
...
causes two of the 401KCoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the 401KCoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the 401KCoin source code).
