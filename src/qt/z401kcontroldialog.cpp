...
#include "ui_z401kcontroldialog.h"
...
std::set<std::string> Z401kControlDialog::setSelectedMints;
std::set<CMintMeta> Z401kControlDialog::setMints;

Z401kControlDialog::Z401kControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Z401kControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
Z401kControlDialog::~Z401kControlDialog()
{
    delete ui;
}

void Z401kControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void Z401kControlDialog::updateList()
{
...
void Z401kControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void Z401kControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZ401k_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZ401kControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> Z401kControlDialog::GetSelectedMints()
{
...
void Z401kControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
