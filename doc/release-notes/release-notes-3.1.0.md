401KCoin Core version *3.1.0* is now available from:  <https://github.com/401kcoin/401kcoin/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/401kcoin/401kcoin/issues>


Mandatory Update
==============

401KCoin Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
z401K Staking is here! z401K staking will be activated on the morning of the 8th of May 2018. With the release of z401K staking, there are effectively 2 versions of z401K, z401K minted on the 3.0.6 401KCoin wallet or lower, and z401K minted on 401KCoin wallet version or higher. New features in this release will require the use of z401K v2, z401K minted on this wallet release 3.1.0 or later. If you currently hold z401K v1 and wish to take advantage of z401K staking and deterministic z401K, you will need to spend the z401K v1 to yourself and remint z401K v2.
Note: To find your z401K version, click the privacy tab, then the z401K Control button then expand the arrows next to the desired denomination.


### Deterministic z401K Seed Keys

z401K is now associated with a deterministic seed key. With this seed key, users are able to securely backup their z401K outside of the wallet that the z401K had been minted on. z401K can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated z401K minting

z401K minting now only requires 1 further mint (down from 2) to mature. z401K mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### z401K Search

Users will now have the ability to search the blockchain for a specific serial # to see if a z401K denomination has been spent or not.



PIV/z401K Staking and Masternode Rewards
--------------

### PIV, z401K and Masternode Payment Schedule

To encourage the use of z401K and increase the 401KCoin zerocoin anonymity set, the 401KCoin payment schedule has been changed to the following:

If a user staking z401K wins the reward for their block, the following z401K reward will be: 
- 3 z401K (3 x 1 denominations) rewarded to the staker, 2 PIV rewarded to the masternode owner and 1 PIV available for the budget. This is a total block reward of 6 PIV, up from 5.
...
### Return change to sender when minting z401K

Previously, z401K minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most PIV and return the change from a z401K mint to that address.
...
The visual layout of the 401KCoin Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto z401K minting feature. This is enabled by default and the enablezeromint=0 setting in the 401kcoin.conf file will overwrite the GUI option.
-	The percentage of autominted z401K can now be set from 1 to 100, changed from 10 – 100.
...
### Summary
 ...
 The 401KCoin wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the 401kcoin.conf file, found in the 401KCoin data directory.
- backuppath = <directory / full path>
- z401kbackuppath = <directory / full path>
...
* If z401K backup, auto generated name is `wallet-autoz401kbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
401KCoin Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The 401KCoin client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running 401kcoind without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the 401KCoin support group, which often times sees users say "I didn't spend that z401K". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the 401KCoin masternodes, the monthly budget available to be utilised has been increased to 42,000 PIV / month. This PIV only has the opportunity to be raised once per month (paid to winning proposals) with any unused PIV not created by the blockchain.

### Budget Finalization Fee

The 401KCoin finalization fee for successful proposals has now been reduced, this fee is now 5 PIV down from 50 PIV. The total fee outlay for a successful proposal is now a total of 55 PIV.
...
-	Enforced v1 z401K spends to require a security level of 100
-	Updates to z401K spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the 401KCoin-Qt masternodes tab
-	Updated 401KCoin-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and 401KCoin's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the z401K Control dialog in the QT wallet, a user is able to see z401K marked as version 1 or 2.

zPoS (z401K staking)
--------------

Once a z401K has over 200 confirmations it becomes available to stake. Staking z401K will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination z401K. So for example if a 1,000 z401K denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination z401Ks.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within 401KCoin's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or z401K, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of z401K that can be recalculated at any time using the seed. Deterministic z401K allows for users to backup all of their future z401K by simply recording their seed and keeping it in a safe place (similar to backing up a private key for PIV). The z401K seed needs to remain in the wallet in order to spend the z401K after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private z401K data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

401KCoin's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which 401KCoin used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to 401KCoin using the bug bounty program. The modulus is now correctly imported and 401KCoin's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] z401K v2: zPoS dz401K ez401K external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh z401K balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of z401K status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault 401kcoin-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on z401K spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting z401K. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending z401K. (presstab)
