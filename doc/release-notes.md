401KCoin Core version 3.0.4 is now available from:
...
401KCoin Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the z401K protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
Refactoring of z401k Spend Validation Code
---------------------
z401k spend validation was too rigid and did not give enough slack for reorganizations. Many staking wallets were unable to reorganize back to the correct blockchain when they had an orphan stake which contained a z401k spend. z401k double spending validation has been refactored to properly account for reorganization.

Money Supply Calculation Fix
---------------------
Coin supply incorrectly was counting spent z401k as newly minted coins that are added to the coin supply, thus resulting in innacurate coin supply data.
...
---------------------
The stake miner code now filters out z401k double spends that were rarely being slipped into blocks (and being rejected by peers when sent broadcast).
...
- #294 `27c0943` Add additional checks for txid for z401k spend. (presstab)
- #301 `b8392cd` Refactor z401k tx counting code. Add a final check in ConnectBlock() (presstab)
- #306 `77dd55c` [Core] Don't send not-validated blocks (Mrs-X)
- #312 `5d79bea` [Main] Update last checkpoint data (Fuzzbawls)
- #325 `7d98ebe` Reindex z401k blocks and correct stats. (presstab)
- #327 `aa1235a` [Main] Don't limit z401K spends from getting into the mempool (Fuzzbawls)
...
- #316 `ed192cf` [Minting] Better filtering of z401k serials in miner. (presstab)

### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending z401K (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when 401kcoind --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] 401KCoin not responding on user quitting app (Aaron Langford)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/401kcoin-project-translations/).
